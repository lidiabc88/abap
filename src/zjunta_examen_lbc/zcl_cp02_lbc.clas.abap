...
CLASS zcl_cp02_lbc IMPLEMENTATION.
...
" ---------------------------------------------------------------------
    ...
    " ---------------------------------------------------------------------
    " * Campos con tipos genéricos identificados en CP02:
    "   - id_reserva (i), aerolinea (c), num_vuelo (n), pasajero (string),
    "     fecha (d), precio (p), estado (c).
    " * Beneficios de usar Elementos de Datos en su lugar:
    "   - ZDE_RESERVA_ID_LBC (INT4): Evita discrepancias de tamaño y tipado numérico.
    "   - ZDE_AIRLINE_LBC (CHAR 2): Permite asociar ayudas de búsqueda de aerolíneas globales.
    "   - ZDE_PASSENGER_LBC (CHAR 40): Limita y estandariza el tamaño del nombre en vez de usar string dinámico.
    "   - ZDE_BOOKING_STATUS_LBC (CHAR 1): Permite añadir valores fijos ('A' = Activa, 'C' = Cancelada) en el dominio.
    " ---------------------------------------------------------------------
    ...
    DATA lt_reservas TYPE TABLE OF ty_reserva.
...
( id_reserva = 8 aerolinea = 'AA' num_vuelo = '0064' pasajero = 'Sarah Jones' fecha = '20260525' precio = '510.00' estado = 'A' )
    ...
    DATA lt_activas TYPE TABLE OF ty_reserva.
